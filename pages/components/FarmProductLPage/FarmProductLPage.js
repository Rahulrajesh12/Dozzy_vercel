...
const FarmProductLPage = ({ count,canonicalUrl }) => {
    ...
    useEffect(() => {
        // This effect will handle the loading status based on data fetching or other actions
        // For example, we assume the data is loaded when the component mounts
        // Simulate loading until the page content is ready
        
        const loadData = async () => {
            // Data loading or any async operations would happen here
            // Once data is loaded or page is ready, set loading to false
            setLoading(false);
        };

        loadData(); // Trigger the data loading

    }, []); // Empty dependency array means this effect runs once after the initial render

    useEffect(() => {
        // Handle route change and show loading state
        const handleRouteChange = () => {
            setLoading(true); // Show loading when route is changing
        };

        // Hide the loader once route change is complete
...
<script async src="https://www.googletagmanager.com/gtag/js?id=AW-16698821101"></script>
        ...
        />
            ...
            {!loading && (
                <div className="text-center px-6 pb-10 pt-8">
                    <button className="bg-[#4508a6] text-xl font-bold text-white w-full lg:w-96 py-4 rounded-full">
                        <a href="/explore-all-farmhouses-in-hyderabad">View all farm houses</a>
                    </button>
                </div>
            )}
        </div>
